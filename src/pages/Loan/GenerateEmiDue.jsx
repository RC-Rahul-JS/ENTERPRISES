...
const GenerateEmiDue = () => {
  ...
  const handleGenerate = async () => {
    ...
    const confirm = await Swal.fire({
      ...
      text: `Are you sure you want to generate an EMI for Loan ${loan.loan_number} with due date ${dueDate}?`,
      icon: 'question',
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <div className="bg-gradient-to-r from-blue-700 to-blue-500 rounded-2xl p-6 mb-6 text-white shadow-lg">
        ...
        <p className="text-blue-100 text-sm">Manually generate EMI due entries for active loans</p>
      </div>
      ...
      {result && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 mb-6 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-bold text-emerald-700">EMI Generated Successfully</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
            {[
              ['Loan Number', result.loan_number],
              ['EMI Number', `#${result.emi_number}`],
              ['Due Date', result.due_date],
              ['Installment', `₹${(result.installment||0).toLocaleString()}`],
              ['Principal Due', `₹${(result.principal_due||0).toLocaleString()}`],
              ['Interest Due', `₹${(result.interest_due||0).toLocaleString()}`],
              ['Remaining Balance', `₹${(result.remaining_balance||0).toLocaleString()}`],
          ...
          <button onClick={handleReset} className="mt-4 text-sm text-emerald-700 hover:text-emerald-900 font-bold flex items-center gap-1 transition">
            <RefreshCw className="w-4 h-4" /> Process another EMI
          </button>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 mb-5">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={searching}
                className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors shadow-sm"
              >
            ...
            <>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 mb-5">
                <h2 className="text-sm font-bold text-gray-700 mb-4 uppercase tracking-wide flex items-center gap-2">
                  <Activity className="w-4 h-4 text-blue-500" /> Generate Details
                </h2>
