...
const PnlTAccountView = ({ incomeData, expenditureData, netProfitLoss, totals }) => {
    ...
    const headerClass = "bg-green-700 text-white font-bold text-center text-sm p-3 border-r border-gray-300";
    const totalRowClass = "bg-gray-100 font-extrabold text-base border-t-4 border-b-4 border-gray-500";
    const totalCellClass = "p-3 font-mono text-right text-gray-900 text-sm";
                ...
                <table className="min-w-full divide-x divide-gray-300">
                    ...
                    <tbody>
                        {rows.map((_, index) => {
                            ...
                            const incomeItem = incomeData[incomeData.length - 1 - index];
...
const TradingAccountView = ({ incomeData, expenditureData, netProfitLoss, totals }) => {
    ...
    return (
        <div className="rounded-xl overflow-hidden bg-white shadow-lg border border-gray-200 mb-8">
            <h2 className="bg-green-900 text-white p-4 text-xl font-bold text-center">Trading Account</h2>
            <div className="overflow-x-auto">
                <table className="min-w-full divide-x divide-gray-300">
                    <thead>
                        {/* <tr>
                    ...
                    <tbody>
                        ...
                        {isProfit && ( // Net Profit on EXPENSE/DEBIT side
                            <tr>
                                <td className={`${cellClass} bg-green-100 font-bold text-green-800`}>Profit</td>
                                <td className={`${cellClass} text-right font-mono bg-green-100 font-bold text-green-800`}>
                                    {formatCurrency(balancingFigure)}
                                </td>
                                <td className={cellClass}></td>
                        ...
                        {!isProfit && ( // Net Loss on INCOME/CREDIT side
                            <tr>
                                <td className={cellClass}></td>
                                <td className={`${cellClass} text-right font-mono`}></td>
                                <td className={`${cellClass} bg-red-100 font-bold text-red-800`}>Loss</td>
                                <td className={`${cellClass} text-right font-mono bg-red-100 font-bold text-red-800`}>
                                    {formatCurrency(balancingFigure)}
                                </td>
                            </tr>
                ...
                <table className="min-w-full divide-x divide-gray-300">
                    ...
                    <tfoot>
                        ...
                        <tr className={totalRowClass}>
                            ...
                            <td className={totalCellClass}>
                                {formatCurrency(grandTotal-Math.abs(totals.assets - totals.liabilities - netProfitLoss))}
                            </td>
                        </tr>
...
export default function FinancialStatementsView() {
  const printRef = useRef(null);
    ...
    const TradingDRData = data?.TradingDR || [];
    const TradingCRData = data?.TradingCR || [];

    //trading
    const TradingDRDataTotals = calculateTotalBalance(TradingDRData);
    const TradingCRDataTotals = calculateTotalBalance(TradingCRData);
    const netTProfitLoss = TradingCRDataTotals.balance - TradingDRDataTotals.balance;
  ...
  Download PDF
            ...
            {loading ? (
                ...
                <>
                   ...
                   <TradingAccountView 
                        incomeData={TradingCRData} 
                        expenditureData={TradingDRData} 
                        netProfitLoss={netTProfitLoss}
                        totals={{ income: TradingCRDataTotals.balance, expenditure: TradingDRDataTotals.balance }}
                    />
