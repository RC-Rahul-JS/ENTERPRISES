        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 transform transition-all duration-300 scale-100">
                ...
                <p className="text-gray-700 mb-6">
                    Are you sure you want to permanently delete the client: <span className="font-semibold text-gray-900">{clientName}</span>? This action cannot be undone.
                </p>
...
const ClientTable = ({ clients, onDeleteRequest, onEdit, loading }) => {
    return (
        <div className="bg-white p-6 rounded-xl shadow-lg mt-8 overflow-x-auto">
            <h2 className="text-xl font-semibold mb-4 text-gray-700">Client List ({clients.length})</h2>
            ...
            {loading ? (
                ...
                <table className="min-w-full divide-y divide-blue-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                        {clients.map((client) => (
                            <tr key={client.id} className="hover:bg-blue-50 transition duration-150">
                                {/* Company Name */}
            ...
            {clients.length === 0 && !loading && (
                 <div className="text-center py-8 text-gray-500 italic">No clients found. Add a new client above!</div>
            )}
        </div>
    );
};
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-lg border-t-4 border-blue-500">
            <button 
                ...
                disabled={loading}
            >
                <ChevronLeft size={16} className="mr-1" /> Back to Client List
            </button>

            <h2 className="text-xl font-bold mb-6 text-blue-700 flex items-center">
                {isEditing ? (
                    <><Edit size={20} className="mr-2" /> Edit Client Details: {initialClient.name}</>
                ) : (
                    <><PlusCircle size={20} className="mr-2" /> Enter New Client Details</>
                )}
            </h2>
                ...
                <div className="mt-6 flex justify-end space-x-4 pt-4">
                    ...
                    >
                        {loading ? (
                            ...
                            <><Edit size={18} className="mr-2" /> Update Client</>
                        ) : (
                            <><PlusCircle size={18} className="mr-2" /> Save Client</>
                        )}
                    </button>
                </div>
            </fieldset>
...
const App = () => {
        ...
        setTimeout(async() => {
            if (clientData.id) {
                ...
                setClients(prevClients => 
                 ...
                 const res=await postData('/trade/customer',clientData)
                         console.log(res)
                              Swal.fire({
                                title: "Successful",
                ...
                setClients(prevClients => [...prevClients, { ...clientData, id: newId }]);
                 ...
                 const res=await postData('/trade/customer',{ ...clientData, id: newId, pAccount:'A20' })
                         console.log(res)
                              Swal.fire({
                                title: "Successful",
                                text: 'Submitting Product Data',
            ...
            setLoading(false);
            handleViewCancel(); // Switch back to list view
        }, LOCAL_DELAY);
    };
    ...
    const handleDeleteExecute = () => {
        ...
        setTimeout(() => {
            setClients(prevClients => 
                prevClients.filter(c => c.id !== modalClient.id)
            );
